...
extern "C" int ivl_type_simple_bound_has_prop_offset(ivl_type_t net, unsigned idx)
{
      const netclass_t*class_type = dynamic_cast<const netclass_t*>(net);
      if (class_type == 0)
	    return 0;
      return class_type->get_simple_bound(idx).has_prop_offset ? 1 : 0;
}

extern "C" unsigned ivl_type_simple_bound_sysfunc_type(ivl_type_t net, unsigned idx)
{
      const netclass_t*class_type = dynamic_cast<const netclass_t*>(net);
      if (class_type == 0)
	    return 0;
      return static_cast<unsigned>(class_type->get_simple_bound(idx).sysfunc_type);
}

extern "C" unsigned ivl_type_simple_bound_sysfunc_arg(ivl_type_t net, unsigned idx)
{
      const netclass_t*class_type = dynamic_cast<const netclass_t*>(net);
      if (class_type == 0)
	    return 0;
...
extern "C" int ivl_type_simple_bound_is_excluded_range(ivl_type_t net, unsigned idx)
{
      const netclass_t*class_type = dynamic_cast<const netclass_t*>(net);
      if (class_type == 0)
	    return 0;
      return class_type->get_simple_bound(idx).is_excluded_range ? 1 : 0;
}

extern "C" int64_t ivl_type_simple_bound_excluded_range_low(ivl_type_t net, unsigned idx)
{
      const netclass_t*class_type = dynamic_cast<const netclass_t*>(net);
      if (class_type == 0)
	    return 0;
      return class_type->get_simple_bound(idx).excluded_range_low;
}

extern "C" int64_t ivl_type_simple_bound_excluded_range_high(ivl_type_t net, unsigned idx)
{
      const netclass_t*class_type = dynamic_cast<const netclass_t*>(net);
      if (class_type == 0)
	    return 0;
