 ...
 * Returns true if expression is a simple comparison: property_name OP value
 * On success, fills in: property_name, op_code, const_value
 * If value_expr is provided, it's filled with the elaborated RHS expression
 * for runtime evaluation (non-null means use expr instead of const_value).
                                    ...
                                    int64_t* element_idx = nullptr)
{
      ...
      // Check for array index (e.g., data[0] == 42)
      // The index is stored in name_comp.index
      if (!name_comp.index.empty() && has_element_idx && element_idx) {
 ...
 * Populates the bounds vector with (prop_name, op_code, const_value, is_soft, weight, weight_per_value, source_prop_name, value_expr, has_element_idx, element_idx).
 * The source_prop_name is only used for op_code 7 (property-based size constraints).
 * The value_expr is non-null for runtime variable references (external scope variables).
 * The has_element_idx and element_idx are for indexed array constraints like data[0] == 42.
 */
                                              ...
                                              bool is_soft = false)
{
      ...
      if (dist) {
            ...
            for (auto it = items.begin(); it != items.end(); ++it) {
                  ...
                  // Extract weight from inside_range_t
                  int64_t weight = 1;
                  ...
                  if (rng->single_val) {
                        ...
                        if (num) {
                              ...
                              // op_code 4 is '=' (==)
                              bounds.push_back(std::make_tuple(prop_name, 4, val, is_soft, weight, weight_per_value, perm_string(), (NetExpr*)nullptr, false, (int64_t)0));
                        }
                  } else if (rng->low_val && rng->high_val) {
                        // Range: create >= and <= constraints
                        // For ranges, we need to pair the >= and <= bounds together
                        // so they share the same weight
                        ...
                        if (lo_num) {
                              ...
                              // op_code 2 is 'G' (>=)
                              bounds.push_back(std::make_tuple(prop_name, 2, lo, is_soft, weight, weight_per_value, perm_string(), (NetExpr*)nullptr, false, (int64_t)0));
                        }
                        if (hi_num) {
                              int64_t hi = hi_num->value().as_long();
                              // op_code 3 is 'L' (<=)
                              bounds.push_back(std::make_tuple(prop_name, 3, hi, is_soft, weight, weight_per_value, perm_string(), (NetExpr*)nullptr, false, (int64_t)0));
                        }
                  }
            }
            return;
      ...
      if (foreach_con) {
            ...
            for (const PExpr* body_expr : foreach_con->get_body()) {
                                          ...
                                          if (des && scope) {
                                                ...
                                                if (source_expr) {
                                                      ...
                                                      bounds.push_back(std::make_tuple(target_name, 12, (int64_t)0, is_soft,
                                                                                       (int64_t)1, true, source_name, source_expr, false, (int64_t)0));
                                                      continue;
                                                }
                                          }
                                    }
                  ...
                  bounds.push_back(std::make_tuple(prop_name, op_code, const_val, is_soft,
                                                   (int64_t)1, true, perm_string(), (NetExpr*)nullptr, false, (int64_t)0));
            }
            return;
      }
      ...
      if (analyze_constraint_expr(expr, prop_name, op_code, const_value, des, scope, &source_prop_name, &value_expr, &has_elem_idx, &elem_idx)) {
            ...
            // Elements 9-10 are has_element_idx and element_idx for indexed array constraints
            bounds.push_back(std::make_tuple(prop_name, op_code, const_value, is_soft, (int64_t)1, true, source_prop_name, value_expr, has_elem_idx, elem_idx));
      }
}
					    ...
					    width_mode_t&)
{
      ...
      // Mailbox method width. Mailboxes use IVL_VT_CLASS as base type but are
      // not netclass_t. Check for mailbox before class check.
      if (search_results.net && search_results.net->data_type()==IVL_VT_CLASS
	  && dynamic_cast<const netmailbox_t*>(search_results.net->net_type())) {
	    // num(), try_put(), try_get(), try_peek() return int (32-bit signed)
	    if (method_name == "num" || method_name == "try_put"
		|| method_name == "try_get" || method_name == "try_peek") {
		  expr_type_  = IVL_VT_BOOL;
		  expr_width_ = 32;
		  min_width_  = expr_width_;
		  signed_flag_= true;
	    ...
	    // put(), get(), peek() are tasks, not expressions - they return nothing
	    // But if called in expression context, treat as error
	    return 0;
      }
                                          ...
                                          unsigned flags) const
{
	...
	/* Handle $ivl_std_randomize - Randomize standalone variables with optional
      ...
      if (name=="$ivl_std_randomize") {
	    ...
	    // bounds_per_sig[signal_idx] = vector of (op_code, const_val, weight, weight_per_value, value_expr)
	    std::vector<std::vector<std::tuple<int, int64_t, int64_t, bool, NetExpr*>>> bounds_per_sig(num_signals);
	    ...
	    for (const PExpr* cons : constraints) {
		  ...
		  for (const auto& ex : extracted) {
			...
			// has_element_idx and element_idx (std::get<8> and <9>) not used for std::randomize
			...
			for (unsigned sig_idx = 0; sig_idx < sig_names.size(); sig_idx++) {
			      if (cons_name == sig_names[sig_idx]) {
				    ...
				    bounds_per_sig[sig_idx].push_back(
					  std::make_tuple(op_code, const_val, weight, weight_per_value, value_expr));
				    break;
			      }
			}
		  }
	    ...
	    for (unsigned sig_idx = 0; sig_idx < num_signals; sig_idx++) {
		  ...
		  for (const auto& bound : bounds_per_sig[sig_idx]) {
			...
			// Use runtime expression if available, otherwise use constant
			NetExpr* val_expr = std::get<4>(bound);
			if (val_expr) {
			      sys_expr->parm(parm_idx++, val_expr);
			} else {
						...
						const
{
      ...
      if (search_results.path_tail.size() > 1) {
	    ...
	    if (method_name == "randomize") {
		  ...
		  const std::list<PExpr*>& constraints = get_inline_constraints();
		  // (prop_idx, op_code with soft, const_val, weight, weight_per_value, value_expr, src_prop_idx, has_element_idx, element_idx)
		  std::vector<std::tuple<size_t, int, int64_t, int64_t, bool, NetExpr*, size_t, bool, int64_t>> bounds;
		  ...
		  for (const PExpr* cons : constraints) {
			...
			for (const auto& ex : extracted) {
			      ...
			      NetExpr* value_expr = std::get<7>(ex);
			      bool has_element_idx = std::get<8>(ex);
			      int64_t element_idx = std::get<9>(ex);
			      ...
			      if (pidx >= 0) {
				    ...
				    bounds.push_back(std::make_tuple((size_t)pidx, op_code, const_val, weight, weight_per_value, value_expr, src_prop_idx, has_element_idx, element_idx));
			      }
			}
		  }
		  ...
		  // Format: prop_idx, op_code, value, weight, weight_per_value, source_prop_idx, has_element_idx, element_idx
		  unsigned parm_idx = 1;
		  for (const auto& bound : bounds) {
			...
			sys_expr->parm(parm_idx++, op_const);
			// Bound value - use runtime expression if available
			NetExpr* val_expr = std::get<5>(bound);
			if (val_expr) {
			      sys_expr->parm(parm_idx++, val_expr);
			} else {
			...
			// Has element index flag (for indexed array constraints like data[0] == 42)
			bool has_elem_idx = std::get<7>(bound);
			NetEConst* has_elem_const = new NetEConst(verinum((uint64_t)(has_elem_idx ? 1 : 0), 32));
			sys_expr->parm(parm_idx++, has_elem_const);
			// Element index value
			int64_t elem_idx = std::get<8>(bound);
      ...
      // Mailbox methods - handle try_put(), try_get(), try_peek(), num() which return values
      if (search_results.net
	  && dynamic_cast<const netmailbox_t*>(search_results.net->net_type())
	  && search_results.path_head.back().index.size()==0) {
	    // Get the method name that we are looking for.
	    ...
	    // put(), get(), peek() are tasks, not expressions - they can't return values
	    // They are handled in elaborate.cc PCallTask::elaborate_method_
	    cerr << get_fileline() << ": error: mailbox method " << method_name
		 << " cannot be used in an expression context." << endl;
	    des->errors += 1;
      ...
      if (sub_expr->expr_type()==IVL_VT_CLASS) {
	    ...
	    if (method_name == "randomize") {
		  ...
		  // become AND expressions)
		  const std::list<PExpr*>& constraints = get_inline_constraints();
		  // (prop_idx, op_code with soft, const_val, weight, weight_per_value, source_prop_idx, value_expr, has_element_idx, element_idx)
		  std::vector<std::tuple<size_t, int, int64_t, int64_t, bool, size_t, NetExpr*, bool, int64_t>> bounds;
		  ...
		  for (const PExpr* cons : constraints) {
			...
			for (const auto& ex : extracted) {
			      ...
			      if (pidx >= 0) {
				    ...
				    // Encode soft flag in bit 3 of op_code
				    if (is_soft) op_code |= 8;
				    bounds.push_back(std::make_tuple((size_t)pidx, op_code, const_val, weight, weight_per_value, source_prop_idx, value_expr, has_element_idx, element_idx));
			      }
			}
		  }
		  ...
		  for (const auto& bound : bounds) {
			...
			sys_expr->parm(parm_idx++, op_const);
			// Bound value - use runtime expression if available
			NetExpr* val_expr = std::get<6>(bound);
			if (val_expr) {
			      sys_expr->parm(parm_idx++, val_expr);
			} else {
			      int64_t val = std::get<2>(bound);
      ...
      if (sub_expr->expr_type()==IVL_VT_STRING) {
	    ...
	    if (method_name == "compare") {
		  if (parms_.size() != 1) {
			cerr << get_fileline() << ": error: Method `compare()`"
			     << " requires 1 argument, got " << parms_.size()
			     << "." << endl;
		  }
		  NetESFunc*sys_expr = new NetESFunc("$ivl_string_method$compare",
	    ...
	    if (method_name == "icompare") {
		  if (parms_.size() != 1) {
			cerr << get_fileline() << ": error: Method `icompare()`"
			     << " requires 1 argument, got " << parms_.size()
			     << "." << endl;
		  }
		  NetESFunc*sys_expr = new NetESFunc("$ivl_string_method$icompare",
						     netvector_t::integer_type(), 2);
		  sys_expr->parm(0, sub_expr);
		  if (parms_.size() > 0 && parms_[0].parm) {
			NetExpr*arg = elaborate_rval_expr(des, scope,
				    ...
				    ivl_type_t ntype, unsigned flags) const
{
	...
	// Handle mailbox new(bound) specially - mailbox is a built-in type
      if (dynamic_cast<const netmailbox_t*>(ntype)) {
	    // Mailbox constructor takes an optional bound (default 0 = unbounded)
	    NetExpr*bound_expr = nullptr;
	    if (parms_.size() > 0 && parms_[0].parm) {
		  bound_expr = parms_[0].parm->elaborate_expr(des, scope,
			  &netvector_t::atom2s32, 0);
	    }
	    if (bound_expr == nullptr) {
		  // Default bound is 0 (unbounded)
		  bound_expr = new NetEConst(verinum((uint64_t)0, 32));
		  bound_expr->set_line(*this);
	    }

	    // Create mailbox via system function
	    // Returns the mailbox type (which is class-like)
	    NetESFunc*sys_expr = new NetESFunc("$ivl_mailbox_new", ntype, 1);
	    sys_expr->set_line(*this);
	    sys_expr->parm(0, bound_expr);
	    return sys_expr;
      }
