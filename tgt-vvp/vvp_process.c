 ...
 */
static ivl_type_t load_lval_object_chain_r(ivl_lval_t lval, int access_base_prop, int skip_current_prop)
{
      ...
      if (lval_sig) {
	    ...
	    /* This is a recursive call - access property for chain continuation */
      ...
      assert(lval_nest);
      /* Recursive call - access property at base level */
      ivl_type_t sub_type = load_lval_object_chain_r(lval_nest, 1, 0);
      assert(ivl_type_base(sub_type) == IVL_VT_CLASS);

      /* If skip_current_prop is true, just return the containing type */
...
/*
 * Public entry point for load_lval_object_chain.
...
static ivl_type_t load_lval_object_chain(ivl_lval_t lval)
{
      ...
      return load_lval_object_chain_r(lval, 0, 1);
}
...
static int show_system_task_call(ivl_statement_t net)
{
      ...
      if (strcmp(stmt_name,"$ivl_queue_method$reverse") == 0) {
	    show_stmt_file_line(net, "queue: reverse");
	    ...
	    /* Handle queue property on class objects (obj.queue_prop.reverse) */
	    if (ivl_expr_type(parm0) == IVL_EX_PROPERTY) {
		  draw_eval_object(parm0);  /* Push queue object to stack */
		  fprintf(vvp_out, "    %%qprop/reverse; // Queue property reverse (stub)\n");
		  fprintf(vvp_out, "    %%pop/obj 1, 0;\n");
		  return 0;
	    }
	    ...
	    assert(ivl_expr_type(parm0) == IVL_EX_SIGNAL);
	    ivl_signal_t var = ivl_expr_signal(parm0);
	    fprintf(vvp_out, "    %%qreverse v%p_0;\n", var);
	    return 0;
      }

      if (strcmp(stmt_name,"$ivl_queue_method$sort") == 0) {
	    show_stmt_file_line(net, "queue: sort");
	    ...
	    /* Handle queue property on class objects (obj.queue_prop.sort) */
	    if (ivl_expr_type(parm0) == IVL_EX_PROPERTY) {
		  draw_eval_object(parm0);  /* Push queue object to stack */
		  fprintf(vvp_out, "    %%qprop/sort; // Queue property sort (stub)\n");
		  fprintf(vvp_out, "    %%pop/obj 1, 0;\n");
		  return 0;
	    }
	    ...
	    assert(ivl_expr_type(parm0) == IVL_EX_SIGNAL);
	    ivl_signal_t var = ivl_expr_signal(parm0);
	    fprintf(vvp_out, "    %%qsort v%p_0;\n", var);
	    return 0;
      }

      if (strcmp(stmt_name,"$ivl_queue_method$rsort") == 0) {
	    show_stmt_file_line(net, "queue: rsort");
	    ...
	    /* Handle queue property on class objects (obj.queue_prop.rsort) */
	    if (ivl_expr_type(parm0) == IVL_EX_PROPERTY) {
		  draw_eval_object(parm0);  /* Push queue object to stack */
		  fprintf(vvp_out, "    %%qprop/rsort; // Queue property rsort (stub)\n");
		  fprintf(vvp_out, "    %%pop/obj 1, 0;\n");
		  return 0;
	    }
